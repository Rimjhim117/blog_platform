...
const Profile = () => {
  ...
  const fetchSavedPosts = async () => {
    try {
      ...
      const response = await axios.get('/posts/bookmarks');
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="flex border-b border-gray-200 mb-12">
          <button
            ...
            className={`pb-4 px-6 font-bold text-lg transition-all duration-300 border-b-2 ${
              activeTab === 'my-posts'
                ? 'border-pink-500 text-pink-600 font-extrabold'
                : 'border-transparent text-gray-500 hover:text-gray-800'
            }`}
          >
            My Stories ({myPosts.length})
          </button>
          <button
          ...
          >
            Saved Stories ({savedPosts.length})
          </button>
        </div>
        ...
        {error ? (
          ...
          <div className="text-center py-16">
            <div className="bg-white rounded-3xl shadow-xl p-12 max-w-lg mx-auto border border-purple-100">
              <div className="text-6xl mb-6">🔖</div>
              <h3 className="text-2xl font-bold text-gray-800 mb-4">No saved stories</h3>
              <p className="text-gray-600 mb-8 leading-relaxed">
                Bookmark articles from your feed to keep them here for quick access later.
              </p>
              <Link
                to="/blog"
                className="bg-gradient-to-r from-purple-400 to-indigo-500 text-white px-8 py-4 rounded-full hover:from-purple-500 hover:to-indigo-600 transition duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 inline-block font-medium"
              >
                Browse Stories
              </Link>
            </div>
          ...
          <div className="space-y-8">
            {savedPosts.map((post) => (
              <div key={post._id} className="bg-white rounded-3xl shadow-lg hover:shadow-xl transition duration-300 border border-purple-100">
                <div className="p-8">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between">
                    {/* Post Info */}
                    <div className="flex-1 mb-6 lg:mb-0 lg:mr-8">
                      <Link
                        ...
                        className="block group"
                      >
                        <h3 className="text-2xl font-bold text-gray-800 mb-2 group-hover:text-purple-500 transition duration-300">
                          {post.title}
                        </h3>
                      </Link>
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-4">
                          {post.tags.slice(0, 3).map((tag, index) => (
                            <span
                              key={index}
                              className="bg-gradient-to-r from-purple-100 to-indigo-100 text-purple-700 text-xs px-3 py-1 rounded-full border border-purple-200"
                            >
                              #{tag}
                            </span>
                          ))}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button
                        ...
                        className="bg-gradient-to-r from-purple-100 to-purple-200 text-purple-700 px-4 py-2 rounded-full hover:from-purple-200 hover:to-purple-300 transition duration-300 text-sm font-medium border border-purple-200 flex items-center"
                      >
                        Remove Bookmark
                      </button>
                    </div>
